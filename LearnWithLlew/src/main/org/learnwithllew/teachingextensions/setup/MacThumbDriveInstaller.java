...
public class MacThumbDriveInstaller
{
  ...
  private void launch()
  {
    String command = "java -jar Learn_With_Llew_Launcher.jar";
    try
    {
      Process exec = Runtime.getRuntime().exec(command, null,
          new File(getDesktop().getAbsolutePath() + "/LearnWithLlew.Java-master"));
    }
    catch (IOException e)
    {
  ...
  {
    File zipFile = new File("./LearnWithLlew.Java-master.zip");
